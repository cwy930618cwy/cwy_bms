  ...
  <div class="tableList">
    <el-form ref="formName" :model="formData" label-width="80px">
      <div v-for="(item, index) in formData.formList" :key="index">
        ...
        >
          <!-- input输入框 -->
          <el-input v-if="item.type === 'Input'" :placeholder="item.placeholder" v-model="item.data"></el-input>
          <!-- 下拉框 -->
          <el-select v-if="item.type === 'Select'" @change="$emit('handle-select', $event)" v-model="item.data" placeholder="请选择活动区域">
          ...
          <!-- 多行输入框 -->
          <el-input v-if="item.type === 'Textarea'" :placeholder="item.placeholder" type="textarea" v-model="item.data"></el-input>
          <!-- 单选 -->
          <div :style="{ textAlign: 'left' }" v-if="item.type === 'Radio'">
            <el-radio v-for="(childItem, childIndex) in item.label" :key="childIndex" v-model="item.data" :label="childItem.value">{{childItem.label}}</el-radio>
          </div>
          <!-- 多选 -->
          <el-checkbox-group v-if="item.type === 'Checkbox'" v-model="item.data">
            <el-checkbox v-for="(childItem, childIndex) in item.defaultdata" :key="childIndex" :label="childItem.value">{{childItem.label}}</el-checkbox>
          </el-checkbox-group>
          <!-- 穿梭框 -->
