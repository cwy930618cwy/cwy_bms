import Vue, { FunctionalComponentOptions, Component as VueComponent } from "vue";
...
const makeDialog = (params: DialogParams & { body?: VueComponent }, close: Function, onButtonClick: Function): {
  ...
  return {
    ...
    componentOptions: {
      ...
      functional: true,
      render (h) {
        return h(Dialog, { props, on: { close, 'button-click': onButtonClick } }, [h(params.body)])
      }
    }
  }
}
...
export default {
  install (V: typeof Vue) {
    ...
    V.prototype.$showDialog = function ({
      ...
      const { update, componentOptions } = makeDialog({ title, buttons, body }, close, onButtonClick)
      this.$modal.show(
        componentOptions,
        {},
